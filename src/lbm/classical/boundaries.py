"""Classical boundary conditions: free-stream far field and bounce-back."""
...
def bounce_back(f: np.ndarray, obstacle: np.ndarray) -> None:
    ...
    For each solid node, reverse all populations: ``f_i <- f_{opp(i)}``.
    """
...
def apply_far_field(
    ...
    """Set all four outer edges to free-stream Shan-He equilibrium (in-place)."""
...
def fluid_state(
    f: np.ndarray, obstacle: np.ndarray
